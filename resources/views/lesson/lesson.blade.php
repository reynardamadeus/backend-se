...
<body class="bg-white min-h-screen flex flex-col">
    ...
    <div class="bg-sky-300 w-full h-24 flex items-center justify-between"><!--batas header-->
        <div class="flex items-center bg-sky-300 p-3 rounded-md w-fit mr-18">
            <label for="level" class="text-black text-sm md:text-lg font-medium mr-2">Select Level:</label>
            <div class="relative">
                <button id="level"
                    class="appearance-none bg-sky-200 text-black text-sm md:text-lg font-medium px-2 py-2 rounded-lg pr-20 cursor-pointer"
            ...
            <aside class="courseVideos border-r-2 border-black w-1/5">
                <div>
                    ...
                    <div class="flex items-center justify-center flex-col gap-3 p-2">
                        ...
                        @empty
                            <p class="text-sm font-semibold text-gray-700">No materials added yet</p>
                        @endforelse
                        @if (Auth::user() != null)
                    ...
                    <x-lesson.create-subject-popup material-class="{{$materials->id}}"/>
                </div>
            </aside>
            ...
            <div class="materi_pelajaran" id="content">
                ...
                <div id="lesson-{{ $l->id }}" class="lesson-content hidden">
                    ...
                    <div class="prose p-4">
                        ...
                        @if (Auth::user() != null)
                            @if(Auth::user()->role == "Admin")
                                <a href="{{route('lesson.content.form', $l->id)}}" class="flex items-center justify-center py-3 bg-sky-300 text-black font-normal rounded-xl shadow-md p-3 hover:bg-blue-700 cursor-pointer">Create Content</a>
                            @else
